...
> `safe_path()` 返回 `SafePath{ wksp, abs, rel }`：`wksp` 是工作区绝对路径
> （交给子进程做输出基准），`abs` 是请求路径的绝对路径（实际读写磁盘用），
> `rel` 是相对工作区的规范化路径；`glob`/`grep` 用 `wksp` + `rel` 让输出
> 前缀保持简短。
